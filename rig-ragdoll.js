(() => {
  ...
  const pointer = {
    ...
    vx: 0, vy: 0, inside: false, down: false
  };
  ...
  function constraintLength(a, b) {
    const pa = posePoint(a);
    const pb = posePoint(b);
    return Math.hypot(pb.x - pa.x, pb.y - pa.y);
  }

  function rebuildConstraintLengths() {
    constraints.forEach((constraint) => {
      constraint.length = constraintLength(constraint.a, constraint.b);
    });
  }

  function fitCanvas() {
    // clientWidth/clientHeight are layout dimensions and are NOT changed by the
    // project card's CSS 3D tilt. This prevents transformed bounding-box jitter from
    // ever being mistaken for a real canvas resize.
    const nextWidth = Math.max(1, canvas.clientWidth || container.clientWidth || 1);
    const nextHeight = Math.max(1, canvas.clientHeight || container.clientHeight || 1);
    const oldWidth = width;
    const oldHeight = height;
    const logicalResize = Math.abs(nextWidth - width) > .5 || Math.abs(nextHeight - height) > .5;
    ...
    if (logicalResize) {
      ...
      // A genuine responsive resize must preserve the current ragdoll pose. Never
      // call resetRagdoll() after initialization just because the layout changed.
      if (initialized && oldWidth > 1 && oldHeight > 1) {
        const sx = width / oldWidth;
        const sy = height / oldHeight;
        Object.values(joints).forEach((joint) => {
          joint.x *= sx; joint.px *= sx;
          joint.y *= sy; joint.py *= sy;
        });
        if (Number.isFinite(pointer.x)) {
          pointer.x *= sx; pointer.px *= sx;
          pointer.y *= sy; pointer.py *= sy;
        }
        rebuildConstraintLengths();
      }
    } else {
      width = nextWidth;
      height = nextHeight;
  ...
  function resetRagdoll() {
    ...
    [...bonePairs.map((pair) => [...pair, false]), ...structuralPairs.map((pair) => [...pair, true])]
      .forEach(([a, b, structural]) => {
        constraints.push({ a, b, structural, length: constraintLength(a, b) });
      });
  ...
  function localPointer(event) {
    // offsetX/Y stay in the canvas' own coordinate system while its parent card tilts.
    // Fall back to rect mapping only for unusual pointer-capture implementations.
    let x = event.offsetX;
    let y = event.offsetY;
  ...
  function nearestJoint(x, y, maxDistance = 28) {
    let best = null;
  ...
  canvas.addEventListener('pointerenter', (event) => {
    ...
    pointer.inside = true;
    hoveredJoint = initialized ? nearestJoint(p.x, p.y) : null;
  ...
  canvas.addEventListener('pointermove', (event) => {
    ...
    if (grabbedJoint) {
      ...
      hoveredJoint = nearestJoint(pointer.x, pointer.y);
      canvas.style.cursor = hoveredJoint ? 'grab' : 'default';
    }
    // Deliberately do NOT stop propagation: the normal project-card tilt should keep working.
  }, { passive: true });
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    const hit = nearestJoint(p.x, p.y, 30);
    if (!hit) return;
  ...
  function releaseJoint(event, cancelled = false) {
    ...
    if (!cancelled) {
      ...
      // A browser pointer-cancel should simply let go; it must never reset the pose.
      grabbedJoint.px = grabbedJoint.x;
      grabbedJoint.py = grabbedJoint.y;
    }
    ...
    hoveredJoint = pointer.inside ? nearestJoint(pointer.x, pointer.y) : null;
    canvas.style.cursor = hoveredJoint ? 'grab' : 'default';
  }

  canvas.addEventListener('pointerup', (event) => releaseJoint(event, false));
  canvas.addEventListener('pointercancel', (event) => releaseJoint(event, true));
  canvas.addEventListener('lostpointercapture', (event) => {
    if (grabbedJoint) releaseJoint(event, true);
  });
  ...
  canvas.addEventListener('pointerleave', () => {
    ...
    // Leaving the card never resets the body.
  });
  ...
  function blendPose(a, b, t, name) {
    ...
    return { x: pa.x + (pb.x - pa.x) * eased, y: pa.y + (pb.y - pa.y) * eased };
  }
  ...
  function solveConstraints() {
    for (let iteration = 0; iteration < 10; iteration += 1) {
      ...
      if (grabbedJoint) {
        ...
        grabbedJoint.y = Math.max(0, Math.min(height, pointer.y));
      }
    }
  }
  ...
  function drawOctaBone(aName, bName, alpha = 1) {
    ...
    ctx.lineTo(p4.x,p4.y); ctx.lineTo(p5.x,p5.y); ctx.lineTo(p6.x,p6.y); ctx.closePath();
    ...
    ctx.beginPath();
    ctx.moveTo(p1.x,p1.y); ctx.lineTo(p2.x,p2.y); ctx.lineTo(p3.x,p3.y);
    ctx.lineTo(p4.x,p4.y); ctx.lineTo(ridge.x,ridge.y); ctx.closePath();
    ...
    ctx.lineWidth = .75;
    ctx.beginPath();
    ctx.moveTo(p1.x,p1.y); ctx.lineTo(ridge.x,ridge.y); ctx.lineTo(p4.x,p4.y); ctx.stroke();
  ...
  function draw(now) {
    ...
    ctx.lineWidth = .8;
    ctx.beginPath();
    ctx.moveTo(width*.08,floorY+.5); ctx.lineTo(width*.92,floorY+.5); ctx.stroke();
